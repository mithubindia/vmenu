...
show_installation_options() {
    ...
    local menu_title="vmenu Installation"
    local menu_text="Choose installation type:"
    
    if [ "$current_install_type" != "none" ]; then
        case "$current_install_type" in
            "translation")
                menu_title="vmenu Update - Translation Version Detected"
                ;;
            "normal")
                menu_title="vmenu Update - Normal Version Detected"
                ;;
            "unknown")
                menu_title="vmenu Update - Existing Installation Detected"
                ;;
        esac
    fi
    
    INSTALL_TYPE=$(whiptail --backtitle "vmenu" --title "$menu_title" --menu "\n$menu_text" 14 70 2 \
...
install_proxmenu() {
    ...
    case "$INSTALL_TYPE" in
        "1")
            show_proxmenux_logo
            msg_title "Installing vmenu - Normal Version"
            install_normal_version
            ;;
        "2")
            show_proxmenux_logo
            msg_title "Installing vmenu - Translation Version"
            install_translation_version
            ;;
        *)
    ...
    msg_title "$(translate "vmenu has been installed successfully")"
    echo -ne "${GN}"
    type_text "$(translate "To run vmenu, simply execute this command in the console or terminal:")"
    echo -e "${YWB}    menu${CL}"
    echo
}
