...
# for Virtuliser VE via dialog-based UI.
...
show_system_commands() {
    while true; do
        ...
        echo "----------------------------------------"
        echo -e " 1) ${GREEN}pveversion${NC}                         - $(translate 'Show Virtuliser version')"
        echo -e " 2) ${GREEN}pveversion -v${NC}                      - $(translate 'Detailed Virtuliser version info')"
        echo -e " 3) ${GREEN}systemctl status pveproxy${NC}          - $(translate 'Check Virtuliser Web UI status')"
...
show_storage_commands() {
    while true; do
        ...
        echo -e "16) ${GREEN}cat /etc/pve/storage.cfg${NC}    - $(translate 'Show Virtuliser storage configuration')"
...
show_network_commands() {
    while true; do
        ...
        echo -e "14) ${GREEN}pve-firewall status${NC}            - $(translate 'Check Virtuliser firewall status')"
        echo -e "15) ${GREEN}pve-firewall compile${NC}           - $(translate 'Compile firewall rules for all nodes')"
        echo -e "16) ${GREEN}pve-firewall reload${NC}            - $(translate 'Reload Virtuliser firewall rules')"
        echo -e " ${DEF}0) $(translate ' Back to previous menu or Esc + Enter')"
        echo
        echo -en "${TAB}${BOLD}${YW}${HOLD}$(translate 'Enter a number, or write or paste a command: ') ${CL}"
        read -r user_input
...
show_update_commands() {
    while true; do
        ...
        echo -e " 3) ${GREEN}pveupdate${NC}                      - $(translate 'Update Virtuliser package lists')"
        echo -e " 4) ${GREEN}pveupgrade${NC}                     - $(translate 'Show available Virtuliser upgrades')"
