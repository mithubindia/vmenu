...
remove_subscription_banner() {
    ...
    if grep -q "NoMoreNagging" "$JS_FILE" 2>/dev/null && [[ -f "$APT_HOOK" ]]; then
        ...
        # Ask user
        if ! whiptail --title "$(translate "Virtuliser Subscription Banner")" \
            --yesno "$(translate "Do you want to remove the Virtuliser subscription banner from the web interface?")" 10 60; then
